...
function getSuggestions(value) {
  ...
  const wildFlares = R.filter(R.propEq('power','Wild'), cosmicFlaresData);
...
class FlareCardContainer extends Component {
  render() {
    ...
    let isWild = R.whereEq({"name":this.props.cardShown, "power":"Wild"});
    let isSuper = R.whereEq({"name":this.props.cardShown, "power":"Super"});

    let wildFlareCard = R.filter(isWild, cosmicFlaresData)[0];
    let superFlareCard = R.filter(isSuper, cosmicFlaresData)[0];
